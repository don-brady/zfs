...
const dmu_object_type_info_t dmu_ot_zt[DMU_OT_NUMTYPES] = {
...
#define	ZDB_OT_NAME(idx) ((idx) < DMU_OT_NUMTYPES ?	\
	dmu_ot_zt[(idx)].ot_name : DMU_OT_IS_VALID(idx) ?	\
...
void
ztu_for_each_zap(objset_t *os, uint64_t object,
    ztu_for_each_zap_cb_t *callback, void *arg)
{
	...
	for (zap_cursor_init(&zc, os, object);
		...
		/* ToDo: allow a way to pass back string? */
		if (attr.za_num_integers != 1 || attr.za_integer_length != 8)
			continue;

		(void) zap_lookup(os, object, attr.za_name, 8, 1, &value);

		callback(attr.za_name, value, index++, arg); 
	}
	zap_cursor_fini(&zc);
}
...
const char *
ztu_object_type(void *handle, uint64_t object)
{
	objset_t *os = (objset_t *)handle;
	dmu_buf_t *db = NULL;
	dmu_object_info_t doi;
	dnode_t *dn;
	...
	if (object == 0) {
		...
		if (dmu_bonus_hold(os, object, FTAG, &db) != 0)
			return (NULL);
		bonus = db->db_data;
		bsize = db->db_size;
	...
	return (zdb_ot_name(doi.doi_type));

//	return (ZDB_OT_NAME(doi.doi_type));
}

boolean_t
ztu_object_isdir(void *handle, uint64_t object)
{
	const char *type = ztu_object_type(handle, object);

	if (type == NULL)
		return (B_FALSE);

	if ( strcmp(type, "zap") == 0 ||
	    strcmp(type, "bpobj") == 0 ||
	    strcmp(type, "Pool properties") == 0 ||
	    strcmp(type, "DSL directory") == 0)
		return (B_TRUE);
	else
		return (B_FALSE);
}
    ...
    uint64_t *dnode_slots_used)
{
	...
	if (verbosity >= 4) {
		...
		(void) printf("***** bonus type %d, doi type %d\n",
		    (int)doi.doi_bonus_type, (int)doi.doi_type);
		*print_header = 1;
	}
...
typedef struct match_objset_cb {
	uint64_t	cb_object;
	char 		*cb_bufptr;
	size_t 		cb_buflen;
} match_objset_cb_t;
...
/*ARGSUSED*/
static int
match_objset(const char *dsname, void *arg)
{
	int error;
	objset_t *os;
	match_objset_cb_t *cbp = (match_objset_cb_t *)arg;
	...
	/* match by object id */
	if (dmu_objset_id(os) == cbp->cb_object) {
		(void) strncpy(cbp->cb_bufptr, dsname, cbp->cb_buflen);
		error = EEXIST;
	}
...
int
ztu_objset_name(uint64_t object, char *bufptr, size_t buflen)
{
	match_objset_cb_t cb;
	int error;
	...
	error = dmu_objset_find(spa_name(spa_tour), match_objset, &cb,
	    DS_FIND_SNAPSHOTS | DS_FIND_CHILDREN);

	return (error == EEXIST ? 0 : ENOENT);
}

int
ztu_open_objset(const char *name, void **handle, int *count)
{
	objset_t *os;
	dmu_objset_stats_t dds;
	uint64_t refdbytes, usedobjs, scratch;
	...
	if (strcmp(name, "$MOS") == 0) {
		os = spa_get_dsl(spa_tour)->dp_meta_objset;
	} else if ((error = dmu_objset_own(name, DMU_OST_ANY, B_TRUE, B_FALSE, "ztour", &os)) != 0) {
		(void) printf("Could not open %s, error %d\n", name, error);
		return (-1);
	}
	...
	if (dds.dds_type == DMU_OST_META) {
		dds.dds_creation_txg = TXG_INITIAL;
		usedobjs = BP_GET_FILL(os->os_rootbp);
		refdbytes = dsl_dir_phys(os->os_spa->spa_dsl_pool->dp_mos_dir)->dd_used_bytes;
	} else {
		dmu_objset_space(os, &refdbytes, &scratch, &usedobjs, &scratch);
	}
	...
	*count = usedobjs;
	...
	(void) printf("zdb_open_objset: \"%s\" type %s\n", name, type);

	return (0);
}

int
ztu_close_objset(void *handle)
{
	objset_t *os = handle;

	if (os != spa_get_dsl(spa_tour)->dp_meta_objset)
		dmu_objset_disown(os, B_FALSE, "ztour");
...
find_zpool(char **target, nvlist_t **configp, int dirc, char **dirv)
{
	...
	(void) fprintf(stderr, "find_zpool ('%s')\n", *target);
...
void
ztu_init(const char *pool)
{
	struct rlimit rl = { 1024, 1024 };
	nvlist_t *policy = NULL;
	char *searchdirs[1] = {"/var/tmp/disks"};
	int nsearch = 1;
	char *target = (char *)pool;
	int error = 0;

	(void) fprintf(stderr, "zdb_init\n");
	...
	(void) fprintf(stderr, "zdb_init (libzfs)\n");

	if (nvlist_alloc(&policy, NV_UNIQUE_NAME_TYPE, 0) != 0 ||
	    nvlist_add_uint32(policy, ZPOOL_REWIND_REQUEST, ZPOOL_NEVER_REWIND) != 0)
		fatal("internal error: %s", strerror(ENOMEM));

	(void) fprintf(stderr, "zdb_init (policy)\n");

	if (1) {
		nvlist_t *cfg = NULL;
		char *name;

		(void) fprintf(stderr, "zdb_init (if)\n");
		name = find_zpool(&target, &cfg, nsearch, searchdirs);
		(void) fprintf(stderr, "zdb_init (found %s)\n", name ? name : "NULL");
		...
		if (name) {
			...
			error = spa_import(name, cfg, NULL, ZFS_IMPORT_MISSING_LOG);
			(void) fprintf(stderr, "spa_import '%s' err %d\n", name, error);
		}
		if (error == 0) {
			error = spa_open_rewind(target, &spa_tour, "ztour", policy, NULL);
		}
		if (error)
			fatal("can't open '%s': %s", target, strerror(error));
	}

	(void) fprintf(stderr, "zdb_init (done)\n");
}

void
ztu_fini(void)
{
	(void) fprintf(stderr, "zdb_fini\n");

	if (spa_tour) {
		dump_uberblock(&spa_tour->spa_uberblock, "\nUberblock:\n", "\n");
		spa_close(spa_tour, "ztour");
	}
	libzfs_fini(g_zfs);
	kernel_fini();
}
